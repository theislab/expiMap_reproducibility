...
for frac in fractions:
    ...
    for i in range(20):
        ...
        q_intr_cvae = sca.models.TRVAE.load_query_data(
            ...
            intr_cvae,
            unfreeze_ext=True,
            new_n_ext_decoder=n_new,
            new_n_expand_encoder=n_new
        )
        ...
        cond_enriched = any(np.isin(top_cond, idx)) and all(top_cond >= 2.3)
        ...
        b_enriched = any(np.isin(top_b, idx)) and all(top_b >= 2.3)
        print("b enriched:", b_enriched)
        if b_enriched:
            stab["b_cells"][str(frac)] += 1

pickle.dump(stab, open(f"./stab_nomask_top_{n_new}.pkl", 'wb'))
